...
export const KanbanLane = ({ laneId, title, items }: KanbanLaneProps) => {
  ...
  return (
    <div ref={setNodeRef} className="flex w-1/3 flex-col min-h-[10rem] mx-2">
      <div className="font-bold inline-flex justify-center bg-gray-400 dark:bg-gray-800 p-3 rounded-t">
        {title.toUpperCase()}
      </div>
...
const KanbanItem = ({ title, index, parent }: KanBanItemProps) => {
  const { isDragging, attributes, transform, listeners, setNodeRef } =
    useDraggable({
      id: index,
      data: {
        title,
